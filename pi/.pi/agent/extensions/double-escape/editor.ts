...
export class DoubleEscapeEditor implements EditorComponent, Focusable {
	...
	constructor(
		...
		if (!actionBase) {
			throw new Error("double-escape: wrapped editor must expose the CustomEditor action surface");
		}
	...
	handleInput(data: string): void {
		if (
			...
			if (this.pacer.registerEscape(this.now()) && this.getText().length > 0) {
				this.setText("");
				return;
			}
		} else {
...
function asActionEditor(editor: EditorComponent): ActionEditor | undefined {
	...
	return candidate.actionHandlers instanceof Map ? candidate as ActionEditor : undefined;
}
