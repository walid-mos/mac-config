...
export class QuestionnaireState {
	...
	enterTab(index: number): void {
		...
		this.editor.setText(
			this.isOpenEnded(q) || q.allowOther
				? (this.drafts.get(q.id) ?? '')
				: '',
		)
	...
	selectOption(index: number): QuestionnaireEffect[] {
		const q = this.currentQuestion()
		if (!q) return NO_EFFECT
		if (!this.isOpenEnded(q) && index === this.chatActionIndex())
	...
	private restoreInitialState(
		...
		for (const answer of initialState.answers) {
			...
			if (!question || !this.isAnswerCompatible(question, answer))
				continue
			this.answers.set(
				question.id,
				this.restoreAnswerState(question, answer),
			)
		}
	}
	...
	private isAnswerCompatible(question: Question, answer: Answer): boolean {
		...
		if (answer.kind === 'single') {
			...
			if (answer.value === UI_TEXT.noResponse)
				return this.isOpenEnded(question)
	...
	private restoreAnswerState(question: Question, answer: Answer): Answer {
		if (answer.kind === 'single') {
			if (answer.wasCustom) {
				if (
					!(
						this.isOpenEnded(question) &&
						answer.value === UI_TEXT.noResponse
					) &&
					!this.drafts.has(question.id)
				) {
		...
		if (answer.customText && !this.drafts.has(question.id))
			this.drafts.set(question.id, answer.customText)
	...
	private firstUnansweredTab(): number {
		const index = this.questions.findIndex(
			question => !this.answers.has(question.id),
		)
	...
	private initialOptionIndex(q: Question): number {
		...
		if (answer?.kind === 'single' && !answer.wasCustom) {
			...
			if (savedIndex >= 0 && opts[savedIndex]?.value === answer.value)
				return savedIndex
