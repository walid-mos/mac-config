...
Turn the current branch state into a published GitHub PR with a title and description a reviewer can understand without having seen the conversation.
...
2. `gh pr create --title "..." --body "$(cat <<'EOF' ... EOF)"` — always HEREDOC for the body. Add `--draft` / `--base` if requested.
3. Report: PR URL, title, one-line recap of what was pushed.

## Troubleshooting

- `gh` not authenticated → suggest the user run `! gh auth login`.
- No `origin` remote → ask which remote/repo to target; don't create one silently.
