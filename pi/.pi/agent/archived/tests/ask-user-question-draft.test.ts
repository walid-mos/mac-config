...
function testCommittedCustomTextSurvivesTabSwitch(): void {
	...
	const effects = state.submitEditorText(submittedText);
...
function testEmptyOpenEndedSubmitClearsPreviousDraft(): void {
	...
	const submittedText = editor.getText();
	editor.clearLikeRealTui();
	assert.deepEqual(state.submitEditorText(submittedText), ["advance"]);
	advanceToNextQuestion(state);
	state.enterTab(0);
...
test("committed custom text survives the auto-advance tab switch", testCommittedCustomTextSurvivesTabSwitch);
test("open-ended free answer survives the auto-advance", testOpenEndedAnswerSurvivesAdvance);
test("manually erased draft is still discarded", testManualEraseStillClearsDraft);
test("selecting a regular option discards the custom draft", testSelectingRegularOptionClearsCustomDraft);
test("selected regular option is restored on revisit", testSelectedRegularOptionIsRestored);
test("empty submit does not skip the following draft", testEmptySubmitDoesNotSkipFollowingDraft);
test("empty multi-select submit does not skip the following draft", testEmptyMultiSelectSubmitDoesNotSkipFollowingDraft);
test("empty open-ended submit clears the previous draft", testEmptyOpenEndedSubmitClearsPreviousDraft);
test("duplicate option values use the saved index", testDuplicateOptionValuesUseSavedIndex);
