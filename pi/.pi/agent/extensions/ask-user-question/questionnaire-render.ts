...
function pushWrappedWithPrefix(sink: LineSink, prefix: string, text: string, width: number): void {
...
export function renderQuestionnaire(
	...
	lines.push("");
	pushWrappedWithPrefix(sink, " ", theme.fg("dim", helpText(state)), renderWidth);
	lines.push(theme.fg("accent", "─".repeat(renderWidth)));
	return lines;
}
...
function renderTabBar(
	...
	const progress = theme.fg("dim", `  ${Math.min(state.tab + 1, state.totalTabs)}/${state.totalTabs}`);
...
function renderContent(
	...
	if (q && !state.isOnSubmitTab()) {
		const modeHint = q.multiSelect ? theme.fg("muted", " (multiple choice)") : "";
		pushWrappedWithPrefix(sink, " ", theme.fg("text", q.prompt) + modeHint, width);
		sink("");
...
function renderOptionRow(
	state: QuestionnaireState,
	q: Question,
	opt: { value: string; label: string; description?: string; recommended?: boolean; isOther?: boolean },
	index: number,
	editor: Editor,
	theme: QuestionnairePalette,
	width: number,
	...
	if (isOther) {
		renderOtherRow(state, editor, prefix, rowLabel, color, theme, width, sink);
	} else {
		const badge = opt.recommended ? theme.fg("success", " (recommended)") : "";
		pushWrappedWithPrefix(sink, prefix, theme.fg(color, rowLabel + opt.label) + badge, width);
	}
	if (!isOther && opt.description) {
		const descIndent = q.multiSelect ? "         " : "     ";
		pushWrappedWithPrefix(sink, descIndent, theme.fg("muted", opt.description), width);
	}
}
...
function renderOtherRow(
	...
	if (state.cursor !== state.currentOptions().length - 1 || !state.editorHasFocus()) {
		const preview = state.typedPreview(ANSWER_PREVIEW_MAX_LENGTH);
		const shown = preview ? theme.fg("muted", preview) : theme.fg(color, UI_TEXT.otherOptionLabel);
		pushWrappedWithPrefix(sink, prefix, theme.fg(color, rowLabel) + shown, width);
		return;
	}
	...
	if (editor.getText().length === 0) {
		...
		sink(`${rowPrefix}${theme.fg("dim", UI_TEXT.otherPlaceholder)}${editorLines[0] ?? ""}`);
		return;
	}
...
function renderOpenEndedEditor(
	...
	if (editor.getText().length === 0) {
		pushWrappedWithPrefix(sink, theme.fg("accent", "> "), theme.fg("dim", UI_TEXT.otherPlaceholder), width);
		return;
	}
...
function renderSubmitScreen(
	...
	pushWrappedWithPrefix(sink, " ", theme.fg("accent", theme.bold("Ready to submit")), width);
	sink("");
	for (const question of questions) {
		const answer = state.answerFor(question.id);
		if (!answer) continue;
		pushWrappedWithPrefix(sink, " ", answerSummaryLine(question.label, answer, theme), width);
	}
	sink("");
	if (state.allAnswered()) {
		pushWrappedWithPrefix(sink, " ", theme.fg("success", "Press Enter to submit"), width);
		return;
	}
	pushWrappedWithPrefix(sink, " ", theme.fg("warning", `Unanswered: ${state.unansweredLabels().join(", ")}`), width);
}

function answerSummaryLine(questionLabel: string, answer: Answer, theme: QuestionnairePalette): string {
