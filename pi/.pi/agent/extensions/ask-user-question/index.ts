...
type ToolText = { content: { type: "text"; text: string }[]; details: AskResult };
...
function isAskResult(details: unknown): details is AskResult {
	...
	if (!("cancelled" in details) || typeof (details as { cancelled: unknown }).cancelled !== "boolean") return false;
	if (!("answers" in details) || !Array.isArray((details as { answers: unknown }).answers)) return false;
	return true;
}
...
export default function askUserQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		renderResult(result, _options, theme, _context) {
			const details = isAskResult(result.details) ? result.details : undefined;
			if (!details) {
			...
			const lines = details.answers.map((a) => {
				...
				const display = a.kind === "multi" && a.labels.length > 1
					? a.labels.join(", ")
