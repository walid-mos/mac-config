...
test('keeps above-editor widgets in priority order without remounting on updates', () => {
	...
	const ui = {
		...
		setWidget(
			_id: string,
			content:
				| ((host: typeof tui, colors: typeof activeTheme) => Component)
				| undefined,
		): void {
...
test('re-renders with the live ui theme after invalidation', () => {
	...
	const ui = {
		...
		setWidget(
			_id: string,
			content:
				| ((host: typeof tui, colors: unknown) => Component)
				| undefined,
		): void {
			if (content === undefined) {
				component = undefined
	...
	try {
		setOrderedAboveEditorWidget(ui, 'themed', {
			priority: 1,
			render: (_width, theme) => [
				`accent:${String((theme as { accent?: string }).accent)}`,
			],
		})
...
test('surface registry and host are process-global across isolated module instances', async () => {
	...
	const secondStack =
		await import('../extensions/ui/ordered-widget-stack.ts?instance=2')
	const secondSurface = await import('../extensions/ui/surface.ts?instance=2')

	assert.equal(
		secondSurface.surfaceRegistry,
		surfaceRegistry,
		'one process-global registry',
	)
	assert.deepEqual(
		secondStack.HOST_WIDGET_IDS,
		HOST_WIDGET_IDS,
		'one stable host key per placement',
	)
	...
	try {
		...
		assert.deepEqual(
			calls,
			[`mount:${HOST_WIDGET_IDS.aboveEditor}`],
			'host kept for background',
		)
		assert.deepEqual(above.get(HOST_WIDGET_IDS.aboveEditor)?.render(80), [
			'◆ BACKGROUND',
		])
