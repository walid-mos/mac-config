...
# GitHub-specific implementation using shared git-pr-common module

# Get script directory for reliable sourcing
typeset -g GPRC_SCRIPT_DIR="${GPRC_SCRIPT_DIR:-$(dirname "${(%):-%x}")}"

# Source shared utilities
source "${GPRC_SCRIPT_DIR}/git-pr-common.sh"

# ============================================================================
# GITHUB-SPECIFIC VALIDATION
# ============================================================================

# Validate GitHub CLI and dependencies
gprc::validate_dependencies() {
    if ! git_pr_common::validate_base_dependencies; then
        return 1
    fi

    if ! git_pr_common::command_exists gh; then
...
gprc::handle_update_mode() {
    local current_branch="$1"

    echo "🔍 Looking for existing PR on current branch..."

    local existing_pr_info=$(gprc::find_existing_pr "$current_branch" "")
    ...
    local pr_body=$(git_pr_common::create_pr_body "$target_branch" "$current_branch" "PR" "gprc")

    if gprc::create_or_update_pr "update" "$pr_number" "" "$pr_body" "$target_branch" "$current_branch"; then
        echo ""
...
gprc::handle_create_mode() {
    ...
    local existing_pr_info=$(gprc::find_existing_pr "$current_branch" "$target_branch")

    if [ "$existing_pr_info" != "null" ] && [ -n "$existing_pr_info" ]; then
        gprc::handle_existing_pr "$existing_pr_info"
        case $? in
            0) return 0 ;;  # Cancel
            2) return 0 ;;  # Viewed
            1)  # Update
        ...
        local pr_title=$(git_pr_common::generate_ai_title "$target_branch" "$current_branch" "PR")
        echo ""
        echo "🔄 Generating changes analysis..."
        local pr_body=$(git_pr_common::create_pr_body "$target_branch" "$current_branch" "PR" "gprc")

        if gprc::create_or_update_pr "create" "" "$pr_title" "$pr_body" "$target_branch" "$current_branch"; then
            echo ""
...
EOF
    ...
    if ! gprc::validate_dependencies; then
        return 1
    fi
    ...
    if [ "$update_only" = true ]; then
        gprc::handle_update_mode "$current_branch"
    else
        gprc::handle_create_mode "$current_branch"
    fi
}
