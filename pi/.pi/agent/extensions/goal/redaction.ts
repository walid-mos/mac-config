...
const SECRET_NAME =
	'(?:(?:[a-z0-9]+[_-])*(?:api[_-]?key|secret[_-]?access[_-]?key|private[_-]?key|access[_-]?token|auth[_-]?token|refresh[_-]?token|id[_-]?token|token|password|passwd|secret|client[_-]?secret))'
...
export function redactSecrets(text: string): string {
	return text
		...
		.replace(JSON_ASSIGNMENT, `$1${REDACTED}$3`)
