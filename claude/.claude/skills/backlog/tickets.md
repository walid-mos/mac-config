...
`Bloquée par` is written in the body **and** set as a native Plane
dependency — the text is what the implementing agent reads, the edge is what
the tracker enforces. Identifiers are only known after creation, so this
section is filled by an update pass (see [`plane.md`](plane.md)).
...
Report the gate result inside `breakdown.html` (see
[`deliverables.md`](deliverables.md)) as a table — one row per ticket, the
failed check numbers or `ok` — next to the dependency graph, before asking
for approval through the document's gate.
