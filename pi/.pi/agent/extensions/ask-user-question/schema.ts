...
const QuestionOptionSchema = Type.Object({
	...
	recommended: Type.Optional(
		Type.Boolean({ description: "Mark this option as the recommended choice (shows a badge)" }),
	),
});
...
const QuestionSchema = Type.Object({
	...
	multiSelect: Type.Optional(
		Type.Boolean({ description: "Allow selecting multiple options (Space toggles, Enter confirms)" }),
	),
...
export const AskParams = Type.Object({
	questions: Type.Array(QuestionSchema, {
		description: "Questions to ask the user. Ask only what's needed: 2-3 is usually enough, 5 max.",
		maxItems: 8,
	}),
});
