...
A request is billed **the moment it enters the Worker** (invocation). Everything upstream is free. Pay-as-you-go accounts now get a **$10 budget alert by default** (since 2026-06-15) — it still only *notifies*, it never cuts. Consequences that drive every decision:
...
- `limits.subrequests` needs wrangler ≥ 4.62; below that the key is unknown. Paid CPU default is still 30 s (configurable up to **5 min** since 2025-03-25).
...
DOs bill **wall-clock while loaded in memory** (GB-s), not CPU — duration is metered in 128 MB slabs, $12.50/M GB-s. ~$32/mo is the price of **1 GB always-on** (~8 objects), not of a single object; 400 000 GB-s/mo are included, so one always-on object stays inside the free tier. Two mandates:
...
- **Workers AI:** ~×80 between smallest and largest model. Output is ~×8 input on older Llama-class models, closer to ×3–4 on 2026 frontier models. Route to the smallest model that suffices; cache deterministic responses and embeddings (never re-embed the same text).
