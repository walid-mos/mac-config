...
**`pi/` package**: `make pi` stows `~/.pi/agent/settings.json` and the versioned extensions and skills; `make pi-post` installs the CLI (`pnpm add -g @earendil-works/pi-coding-agent`) if missing. Pi auth happens on first launch (`pi` then `/login`) and lives in `~/.pi/agent/auth.json`, never versioned. pi writes through the symlink when a setting changes via `/settings`: the diff shows up directly in the repo — review before committing.
