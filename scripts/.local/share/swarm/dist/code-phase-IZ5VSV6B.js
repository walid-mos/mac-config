...
function buildTaskBatches(tasks, maxBatchSize = 3) {
  ...
  const { inDegree, dependents } = buildDependencyGraph(tasks);
  const batches = [];
...
function attemptDir(sessionDir, batchIndex, attempt) {
  return join(sessionDir, `batch-${batchIndex}`, `attempt-${attempt}`);
}
...
var MAX_RETRIES2 = 2;
var TaskExhaustedError = class extends Error {
  constructor(taskId, attempts, lastFindings) {
...
function propagateFailure(failedTaskId, nodes, dependents) {
...
async function executeDag(ctx, registry, tasks, techStack, specItemContext, testFiles, signal, logger, gitState) {
  if (tasks.length === 0) {
...
`);
    ...
    for (const node of failedNodes) {
      ...
      try {
        ...
        process.stderr.write(`Warning: safety commit failed for ${node.taskId}: ${err.message}
`);
      }
    }
    waveIndex++;
...
`);
  ...
  try {
    const { getChangedFiles: getChangedFiles2 } = await import("./git-operations-PQITR2DQ.js");
    finalChangedFiles = await getChangedFiles2(ctx.projectDir);
  } catch (err) {
...
`);
  ...
  if (!success) {
    ...
    const failedNode = plan.tasks.find((t) => t.id === failedTask.taskId);
...
export {
  ...
  findingSignature,
  isImmediateFailErrorCode2 as isImmediateFailErrorCode,
  isRetryableErrorCode2 as isRetryableErrorCode,
  runCodePhase
};
