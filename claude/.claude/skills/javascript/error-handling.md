...
Subclass `Error`, set `this.name`, carry domain fields (e.g. `ValidationError` with a `field` property).

## Never swallow a caught error

A `catch` that does nothing (or only logs) hides the failure from the caller. Either recover meaningfully, or re-throw - preserve the cause with `{ cause }` (the linter checks that a re-thrown error carries the caught one as `cause`).
...
For language-agnostic error principles (fail-fast, error-first ordering, no ambiguous return values), see the `coding` skill (RULE 1, RULE 11).
