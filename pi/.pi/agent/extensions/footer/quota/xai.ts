...
export async function pollXaiQuota(): Promise<XaiQuota | undefined> {
	...
	if (typeof settings?.subscription_tier_display === "string" && settings.subscription_tier_display) {
	...
	if (monthlyLimit !== undefined && monthlyUsed !== undefined) {
		quota.monthly = {
			used: monthlyUsed,
			limit: monthlyLimit,
			reset: typeof billingConfig?.billingPeriodEnd === "string" ? billingConfig.billingPeriodEnd : "",
		};
	}
	...
	const period = isRecord(creditsConfig?.currentPeriod) ? creditsConfig.currentPeriod : undefined;
	...
	if (periodType === "USAGE_PERIOD_TYPE_WEEKLY" || periodType === "USAGE_PERIOD_TYPE_MONTHLY") {
		...
		if (usedPercent === undefined) {
			...
			usedPercent = cap !== undefined && cap > 0 && used !== undefined ? (used / cap) * 100 : 0;
		}
		quota.pool = {
			usedPercent: Math.min(100, Math.max(0, usedPercent)),
			reset: typeof creditsConfig?.billingPeriodEnd === "string"
		...
		if (prepaidBalance !== undefined && prepaidBalance > 0) quota.prepaidBalance = prepaidBalance;
	}
