...
import { renderFooterLines, type FooterRenderInput } from "../extensions/footer.ts";
import { surfaceLineWidth } from "../extensions/ui/surface.ts";

const STRIP_ANSI = /\u001b\[[0-?]*[ -/]*[@-~]|\u001b\]8;;[^\u0007]*\u0007|\u001b\]8;;\u0007/gu;
...
function sample(overrides: Partial<FooterRenderInput> = {}): FooterRenderInput {
	return {
		...
		statuses: [],
		git: { modified: 2, staged: 1, deleted: 0, untracked: 3, stash: 1, ahead: 1, behind: 0 },
...
test("degrades by hiding meta before ever leaking raw escape fragments", () => {
	...
	assert.equal(joined.includes("\u001b]8;;https://"), false, "OSC 8 target leaked as text");
	assert.ok(!/\u001b\]8;;[^\u0007]*$/.test(joined), "unclosed hyperlink after truncation");
	const plain = joined.replace(STRIP_ANSI, "");
	assert.equal(plain.includes("github.com/acme"), false, "URL must never display as text");
});
...
test("renders a clean repository as a green check", () => {
	const line2 = renderFooterLines(
		sample({
			width: 100,
			git: { modified: 0, staged: 0, deleted: 0, untracked: 0, stash: 0, ahead: 0, behind: 0 },
			pr: null,
		}),
	)[1] ?? "";
