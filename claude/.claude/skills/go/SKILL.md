...
name: go
description: >-
  Two modes for shipping a Linear backlog phase. SETUP mode (`/go` or
  `/go <phase-number>`) fetches open issues in the current project's Linear
  backlog, groups them by `[P{N}-...]` prefix, asks the user which phase to
  pick, writes a temp `plan.md` checklist + sibling `tasks.json`, then HANDS
  OFF. NEXT mode (`/go next`) is a two-step ritual per task: it first
  validates the previous task (Linear -> Done + archived, plan tick `[~]`
  -> `[x]`) if one is pending, then picks the next `[ ]`, sets Linear to
  In Progress, implements + verifies + commits + pushes + ticks `[~]`, and
  STOPS. The user paces validation; one `/go next` = one task started, the
  next `/go next` closes it.
user-invocable: true
