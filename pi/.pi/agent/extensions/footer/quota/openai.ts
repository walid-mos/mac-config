...
export async function pollOpenAIQuota(): Promise<OpenAIQuota | undefined> {
	...
	const accountId =
		readAuthField('openai-codex', 'accountId') ?? accountIdFromToken(token)
	if (!accountId) return undefined
	const usage = await fetchJsonWithHeaders(
...
function parseUsage(raw: unknown): OpenAIQuota | undefined {
	...
	const credits =
		creditsRaw?.has_credits === true
			? finiteNumber(creditsRaw.balance)
			: undefined
	const resetsRaw = isRecord(raw.rate_limit_reset_credits)
		? raw.rate_limit_reset_credits
		: undefined
	const resets = finiteNumber(resetsRaw?.available_count)
	const plan =
		typeof raw.plan_type === 'string' && raw.plan_type
