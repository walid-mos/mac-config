...
export function subjectFor(tool: string, args: Record<string, unknown> | undefined): string {
	const a = args ?? {};
	switch (tool) {
		case "bash":
		...
		case "read": {
			...
			const offset = typeof a.offset === "number" && a.offset > 1 ? ` dès la ligne ${a.offset}` : "";
...
function limitSummary(details: unknown, key: string, unit: string): string | undefined {
...
function truncationInfo(details: unknown): { totalLines?: number; marker: string } {
	if (typeof details !== "object" || details === null) return { marker: "" };
	const truncation = (details as Record<string, unknown>).truncation;
	if (typeof truncation !== "object" || truncation === null) return { marker: "" };
