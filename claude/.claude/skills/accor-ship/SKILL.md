...
- Options `/stack` acceptées telles quelles : `--base` (défaut : `develop`),
...
## Exécution — déléguer à `/stack`

Cette feature se livre via le workflow **`/stack`**, avec les valeurs accor
ci-dessus injectées. Invoquer le skill `stack` et suivre ses phases, en respectant
en plus :
...
- **`/simplify` par maillon** : sur le diff du maillon seul, avant de passer au
  suivant. Ré-commiter les correctifs, re-tester vert.
- **`/simplify` global** : sur le diff complet du stack, avant `submit`. Propager
