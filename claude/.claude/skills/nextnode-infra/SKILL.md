...
- [ ] GitHub workflow calls the correct reusable workflow (`deploy-static.yml` / `deploy.yml` / `publish-package.yml`)
- [ ] For `type=app`: `Dockerfile` at root, minimal `docker-compose.yml` (infra-owned keys absent), app respects `$PORT`
- [ ] `SITE_URL` never hardcoded — set via `process.env.SITE_URL` / auto-injected by infra
- [ ] Secrets declared PER-SERVICE (no top-level `[deploy].secrets` for hetzner-vps)
- [ ] Build-time config through `build_args`, never through `secrets`
- [ ] Cross-service URLs read from env (`process.env.<SIBLING>_URL`), never hardcoded
- [ ] R2/Postgres/Supabase backing services: declared in `nextnode.toml`, env vars injected by infra (see `cicd-checklist.md` for per-service detail)
...
- [ ] No `date-fns`, `luxon`, `dayjs`, or `moment` in `dependencies`/`devDependencies` — if found: remove them and replace usages with `@js-temporal/polyfill` (flag to user before touching app code)
