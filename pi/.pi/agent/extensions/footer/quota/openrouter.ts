...
export async function pollOpenRouterQuota(): Promise<
	OpenRouterQuota | undefined
> {
	...
	const creditsEnvelope = isRecord(creditsResponse)
		? creditsResponse
		: undefined
	const keyEnvelope = isRecord(keyResponse) ? keyResponse : undefined
	const credits = isRecord(creditsEnvelope?.data)
		? creditsEnvelope.data
		: undefined
