...
secrets = [                                  # Optional -- GLOBAL pool (both targets), folded into every service. Entry = must-exist NAME or a {name,generate,length} auto-gen table.
  "PREVIEW_SECRET",                          #   must-exist: operator set it in GitHub
  { name = "JWT_SECRET",  generate = "token",    length = 43 },  # auto-generated base64url (JWT/HS256)
  { name = "DB_PASSWORD", generate = "password", length = 24 },  # auto-generated alphanumeric (DB/service pw)
]
...
[deploy.services.web]                        # Required (Hetzner only). At least one; N services per project accepted. All services in a project MUST share the same `source` (mixing build and upstream is rejected).
source = "build"                             # Optional -- "build" (default) | "upstream"
port = 3000                                  # Optional -- TCP port the container listens on (default: 3000). Injected as `PORT`, drives the compose port mapping + /healthz probe.
url = "example.com"                          # Optional -- hostname for external routing. Must be unique across services AND equal/subdomain of project.domain. When set, the service gets a DNS record + Caddy upstream + host port. Absent = internal-only (reached by siblings as `<service-name>:<port>` on the compose network).
secrets = ["RESEND_API_KEY"]                 # Optional -- string[]. Per-service (least-privilege) RUNTIME secret NAMES (from ALL_SECRETS) projected into THIS service's .env.<name> only. GLOBAL [deploy].secrets are folded in on top; pull pool = their union.
needs = []                                   # Optional -- backing services this workload uses at runtime (e.g. ["postgres"]). Must reference a declared [services.<name>]. Opts the service into that backing service's secrets (e.g. DATABASE_URL). When `[services.postgres]` is declared, exactly one service must declare `needs = ["postgres"]` (selects the migrate image).
depends_on = []                              # Optional -- sibling services this workload starts after (compose depends_on). Each entry MUST reference a declared service. Gating: `service_healthy` when Y is build, `service_started` when Y is upstream.
# build-only fields:
...
build_args = ["ANALYTICS_ID"]                # Optional, build only -- EXTRA GitHub Variable NAMES inlined as docker-bake build args (values from ALL_VARS, fail-loud if absent). SITE_URL is auto-injected, never listed. NEVER put secrets here (they bake into image layers).
# upstream-only fields:
# ref = "ghcr.io/some-org/app:v1.2.3"        # Required when source = "upstream"
# registry_auth_secret = "GHCR_READ_TOKEN"   # Optional, upstream only -- repo/org secret NAME holding the registry password. When set, ALL upstream services in the project MUST reference the SAME secret name (homogeneous registry auth). Omit for public images.

[[services.r2.buckets]]                      # Optional -- per-project R2 buckets (Cloudflare R2). Table-array, one per bucket.
name = "assets"                              # Bucket alias (kebab-case). Materialized as `<projectName>-<environment>-<alias>`.
cdn  = true                                  # Optional -- boolean (default false). true -> public custom domain `assets.cdn.<domain>` + `R2_BUCKET_ASSETS_URL`.
...
interface GeneratedSecretConfig {
  readonly name: string
  readonly generate: SecretGenerator  // "token" = base64url (JWT/HS256); "password" = alphanumeric (DB/service pw)
  readonly length: number              // produced CHARACTER count, 8-256
}
...
interface BaseDeploySection {
  ...
  readonly volumes: ReadonlyArray<DeployVolume>  // Hetzner only -- Cloudflare ignores this.
  // The {name,generate,length} tables parsed out of [deploy].secrets. Empty when
  // every declared secret is must-exist. Consumed by `ensureGeneratedSecrets`;
  // the names also live in `secrets` (the pull pool).
...
interface HetznerVpsDeploySection extends BaseDeploySection {
  ...
  // The pool of GitHub Secret NAMES the pipeline pulls = the GLOBAL [deploy].secrets
  // (injected into every service via `expandServiceSecrets`) UNIONED with every
  // service's own [deploy.services.<name>].secrets (least privilege).
  readonly secrets: ReadonlyArray<string>
  readonly hetzner: HetznerDeployConfig
  // Per-service workloads declared under [deploy.services.<name>], keyed by the
  // KEBAB instance name. At least one entry is required; N services per project
  // are accepted. All services MUST share the same `source` (validated in
...
interface CloudflarePagesDeploySection extends BaseDeploySection {
  ...
  // The pool IS [deploy].secrets directly — a Pages deploy is a single unit with
  // no per-service split.
  readonly secrets: ReadonlyArray<string>
}
