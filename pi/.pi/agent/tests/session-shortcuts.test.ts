...
test("registers /clear as a new-session command", async () => {
	...
	assert.equal(command.description, "Start a new session (keeps current model)");
...
test("session_start restores the handed-off model", async () => {
	...
	writeFileSync(
		handoffFile(),
		JSON.stringify({ provider: "anthropic", id: "claude-test", thinkingLevel: "high" }),
	);
	...
	try {
		...
		assert.deepEqual(setModelCalls, [{ provider: "anthropic", id: "claude-test" }]);
...
test("session_start ignores non-new reasons", async () => {
	...
	writeFileSync(handoffFile(), JSON.stringify({ provider: "anthropic", id: "claude-test" }));

	try {
		await sessionStart(
			{ reason: "resume" },
			{ modelRegistry: { find: () => ({ provider: "anthropic", id: "claude-test" }) } },
		);
