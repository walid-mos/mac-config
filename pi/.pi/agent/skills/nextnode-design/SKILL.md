...
2. Read the main CSS file - check if it declares an `@theme` block mirroring `nextnode-landing/src/styles/global.css` (see [colors.md](colors.md))
...
**Default to scale tokens; arbitrary `[…]` values are deliberate exceptions.** When an off-scale value recurs, promote it to a named `@theme` token (the way shadows already work) instead of repeating the inline form.
...
Logos, icons, and favicons ship from `@nextnode-solutions/brand-assets` as subpath exports. Bumping the package version is the single touchpoint when artwork changes. See [logos.md](logos.md) for full subpath table and usage.
