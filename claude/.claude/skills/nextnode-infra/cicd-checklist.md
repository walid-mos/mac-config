...
- [ ] **Secrets are declared PER-SERVICE** in `[deploy.services.<name>].secrets` (runtime secret NAMES; values live in GitHub Secrets). A top-level `[deploy].secrets` is **REJECTED for hetzner-vps**. Each secret is projected into ONLY the `.env.<name>` of the services entitled to it (least privilege). A backing service's secrets reach a service only when it declares `needs = ["postgres"]`.
