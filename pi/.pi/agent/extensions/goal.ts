 ...
 *
 * After each settled turn the evaluator model — routed through the
 * evaluator configuration — judges the condition against the
 * transcript. not_yet → auto-continue. met / impossible → stop.
 * Two invalid evaluator replies → pause without auto-continuing.
 */
import { Type } from 'typebox'

import { loadEvaluatorCandidates } from './agents/config.ts'
import {
	ABOVE_EDITOR_PRIORITY,
	setOrderedAboveEditorWidget,
} from './ui/ordered-widget-stack.ts'

import type { AgentsConfig } from './agents/model-route.ts'
...
type EvaluatorAttempt = EvaluatorModelIdentity & {
...
export default function goalExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			return {
				content: [
					{ type: 'text', text: `Goal set: ${params.condition}` },
				],
			}
		},
	})
...
function handleCommand(args: string, ctx: ExtensionCommandContext): void {
	...
	if (args.length > MAX_CONDITION_CHARS) {
		ctx.ui.notify(
			`Condition too long (${args.length} > ${MAX_CONDITION_CHARS})`,
			'error',
		)
		return
	}
...
/**
 * Evaluator attempts come from the evaluator configuration in
 * config order, filtered to models that are available, authenticated, and
 * inside the root session scope. Ordered route candidates are the only
 * fallbacks — the same policy as the agents model router.
 */
function resolveEvaluatorAttempts(ctx: ExtensionContext): EvaluatorAttempt[] {
	...
	const candidates = configured.flatMap((candidate): EvaluatorAttempt[] => {
		...
		if (!model || !isEvaluatorUsable(ctx, model, scoped)) return []
...
export function selectEvaluatorAttempts<T extends EvaluatorModelIdentity>(
	...
	return unique.slice(0, MAX_ATTEMPTS)
}

function isEvaluatorUsable(
	ctx: ExtensionContext,
	model: Model<Api>,
	...
	return scoped.has(modelIdentityKey(model))
}

function scopedModelKeys(ctx: ExtensionContext): ReadonlySet<string> {
	return new Set(ctx.scopedModels.map(entry => modelIdentityKey(entry.model)))
}
...
async function judgeCondition(
	...
	return evaluateWithFallback(resolveEvaluatorAttempts(ctx), attempt =>
		ctx.modelRegistry
			.complete(
				attempt.model,
...
export async function evaluateWithFallback<T extends EvaluatorModelIdentity>(
	...
	for (const [index, model] of models.slice(0, MAX_ATTEMPTS).entries()) {
		try {
...
function formatEvaluatorFailure(
	...
	const identity = normalizeDiagnostic(
...
function isEvaluatorVerdict(
	value: unknown,
): value is Exclude<GoalVerdict, 'stuck'> {
...
function collectTranscriptExcerpt(branch: readonly SessionEntry[]): {
	...
	for (const entry of branch.slice(-30)) {
		...
		toolCallCount += countToolCalls(entry.message)
...
export function restoreGoalState(value: unknown): GoalState | null {
	...
	return {
		...
		proofs: isStringArray(value.proofs)
			? normalizeProofs(value.proofs)
			: [],
		status: 'active',
	}
}
...
export function goalChromeLines(state: GoalState, width: number): string[] {
	return [
		...
		state.lastReason
			? `last: ${truncate(state.lastReason, 80)}`
