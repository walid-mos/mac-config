...
- Questions with no dependency between them go in the **same round**, asked
  together in one rich-mode HTML questionnaire (see
  [`deliverables.md`](deliverables.md)) — grouped by domain via `data-group`,
  each pre-answered with your recommendation. The user scans, overrides the
  few disagreements, approves.
...
- After each round, fold the answers in, re-derive the frontier of newly
  ...
  rounds.
- **Chat fallback**: one or two trivial questions surfacing mid-exploration
  may go through `AskUserQuestion` instead of a round document — never more.

## Rules

- **Always propose your recommended answer**, with one sentence of why. Every
  form option carries `data-recommended`; approving the document unchanged
  must be a valid, complete answer.
- **Heavy decisions get a compare-axes form, not bare radios.** If the user
  must weigh options (architecture, data shape, contract), render
  `rq-axes` — options in columns, behaviour/cost/consequence in rows — with a
  schema (`.rq-schema`) when the question is spatial.
...
- **Push back once.** If the user's answer creates a contradiction with an
  ...
  alternative — in the next round's form, stake stated. If they reaffirm, it
  is decided — record it and move on.
...
The final round document ends with a recap section — every decision taken,
one line each, rich-mode editable so the user can still reverse one in place.
Approval of that document is the confirmation. If a decision is reversed,
fold it in and re-ship the recap.
