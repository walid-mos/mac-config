...
name: pr
description: >-
  Open a clean GitHub pull request from the current branch to a target branch
  (default: the repo's default branch). Runs pre-flight hygiene checks
  (pushed, up-to-date, no fixup/WIP commits, no PR already open), generates a
  conventional-commit-aligned title and a structured body (Summary / Changes /
  Test plan), and creates the PR via `gh`. Optional flags: `--draft`,
  `--base <branch>`.
user-invocable: true
---
