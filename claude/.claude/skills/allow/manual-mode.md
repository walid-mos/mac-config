...
| `webfetch <domain>` or `web fetch <domain>` | Extract root domain (see [overview.md](overview.md)) -> `WebFetch(domain:<rootDomain>)` |
| `domain <domain>` or `host <domain>` | Extract root domain (see [overview.md](overview.md)) -> `WebFetch(domain:<rootDomain>)` — also add `*.<rootDomain>` to `sandbox.network.allowedDomains` |
