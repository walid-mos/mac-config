...
description: >-
  ...
  (javascript or typescript). Mechanical rules are linted by
  @nextnode-solutions/standards; this skill covers what the linter cannot judge.
---

# React - Mandatory Rules

Mechanical rules (rules-of-hooks, exhaustive-deps, index keys, one component per file, filename = export, JSX depth 8, 5 props, every `useEffect` flagged as a tripwire) are linted - see `nextnode-standards/oxlint.md`. Fix a violation by redesign, never by suppression. If you delegate React to a subagent, its prompt must make it load these skills first.
