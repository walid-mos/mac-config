...
Le skill `html` (génération de livrables HTML) est requis pour les gates 2–4
mais n'est pas encore importé dans pi. En attendant :

- **Gate 2** : poser les questions via `ask_user_question` (regroupées par round)
- **Gate 3** : écrire la spec en Markdown dans `docs/interviews/<feature-slug>/spec.md`
- **Gate 4** : présenter le breakdown en Markdown, validation via `ask_user_question`

## 1. Orient

- **Resolve the target project.** `plane_list_projects`, match the repo (directory
...
- **Check for prior art on the tracker**: `plane_search_workitems` on the feature's
...
Render it as `docs/interviews/<feature-slug>/spec.md`, get approval. Do not
publish it yet.
...
- Present the breakdown (Markdown) : dependency graph, tickets per epic,
  gate results. Get explicit approval via `ask_user_question`.
