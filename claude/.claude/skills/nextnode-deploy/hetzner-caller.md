...
The instance name (table key, KEBAB) flows end-to-end: the bake target, the GHCR repo suffix (`<owner>/<repo>-<name>`), the `.env.<name>` per-service env file on the VPS, the host-port allocation slot (for routed services), the DNS record + Caddy upstream (for routed services), and the migrate container's image lookup. N services per project are accepted; all services in one project MUST share the same `source` (mixing build and upstream is rejected at parse time).
...
When `source = "upstream"` for every service, **all services that declare `registry_auth_secret` MUST reference the same secret name** (homogeneous registry auth). The forwarded token is used for `docker login` on every registry it covers. Mixing different auth secrets is rejected at deploy-context resolution.
...
There is no caller `docker-compose.yml`. Build and run the image directly with the same Dockerfile CI uses — from the **repo root** (the monorepo build context):

```bash
docker build -f packages/<pkg>/Dockerfile -t <pkg>:local .
docker run --rm -p 3000:3000 -e PORT=3000 <pkg>:local
curl -fsS http://localhost:3000/   # expect 200
```

In CI that same Dockerfile is built by `docker/bake-action` against the rendered `docker-bake.json` and tagged with the GHCR ref. See [/turborepo](../turborepo/SKILL.md#smoke-test) for the canonical monorepo build + smoke test.
...
6. **Monorepo callers delegate the Dockerfile build strategy to `/turborepo`** - the `turbo prune --docker` pattern is provider-agnostic and lives in its own skill. The Hetzner caller only owns the runtime contract (port from config, declared service name, infra-owned compose keys, `/healthz` for build).
7. **Smoke-test the image with `docker build` + `docker run` locally before pushing a deploy** - Definition of Done for any change that triggers `deploy.yml`. See the pre-flight section above and [/turborepo](../turborepo/SKILL.md#smoke-test).
8. **All services in one project share the same `source`** - mixing `build` and `upstream` across services is rejected at parse time. If a project genuinely needs both, split it into two `nextnode.toml`s (two workflows, two VPS silos) — there is no per-service auth/bake escape hatch.
