...
export async function pollKimiQuota(): Promise<KimiQuota | undefined> {
	...
	const response = await fetchJson("https://api.kimi.com/coding/v1/usages", token);
	...
	const fiveHourRaw = limits.find((limit: unknown) => {
		const window = (limit as { window?: { duration?: number; timeUnit?: string } })?.window;
	...
	const weekly = parseWindow(data?.usage) ?? parseWindow(weeklyLimitRaw?.detail ?? weeklyLimitRaw);
	return {
		fiveHour,
...
function parseWindow(detail: unknown, resetFallback?: unknown): KimiQuota["fiveHour"] | undefined {
	...
	const resetRaw = detail.resetTime ?? detail.reset_time ?? detail.resetAt ?? resetFallback;
