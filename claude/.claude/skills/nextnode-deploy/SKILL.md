...
35. **A Worker declares its four barriers in `nextnode.toml`, and two of them are gated by plan** — `rate_limit` (a zone `http_ratelimit` ruleset) and `public_paths` (ONE zone `http_request_firewall_custom` ruleset, blocking the negation of the declared paths) are emitted in PRODUCTION ONLY, since a zone owns a single entry point per phase; `limits` and `[[rate_limiters]]` land in the wrangler config for both environments. Before declaring either gated block: **`rate_limit` needs a Pro zone** (a Free zone allows only the `Path` field in the expression, a 10 s period and 10 s mitigation timeout, and no custom response) and **`limits` needs a Workers Paid account** (`wrangler deploy` fails with `code: 100328` on Free). Free-plan ceilings (one rate limiting rule, five custom rules per zone), the host requirement (a barrier needs `url`) and the path grammar are all refused at config load. See [cloudflare-workers.md](cloudflare-workers.md) and the `cloudflare-cost` skill.
