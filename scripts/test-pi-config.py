...
def require_expected_layout(home: Path) -> None:
    ...
    expected_versioned_skill = agent / "skills"  # raz : aucun skill versionné tant que le rebuild n'a pas commencé
    ...
    if missing_links or linked_runtime_paths or changed_runtime or retained_static_resources or not expected_versioned_skill.is_dir() or not expected_external_skill.is_file() or settings.get("skills") != ["~/.pi/agent/external/skills"]:
        details = []
        if missing_links:
