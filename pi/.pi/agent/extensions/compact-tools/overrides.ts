...
export function createCompactOverrides(options: CompactOverridesOptions): CompactToolDefinition[] {
	const tools = options.tools ?? COMPACT_TOOLS;
	const nativeByCwd = new Map<string, Map<string, CompactToolDefinition>>();

	function nativeFor(tool: string, cwd: string): CompactToolDefinition | undefined {
	...
	for (const tool of tools) {
		...
		overrides.push({
			...
			execute(toolCallId, params, signal, onUpdate, ctx) {
				...
				if (!native) throw new Error(`compact-tools: no native definition for "${tool}"`);
