...
3. **Each task = one commit. Each phase = one PR.** See Rule 3 in the Rules section below for the canonical atomicity contract. Enforcement detail here: if the plan describes "do X and Y", split into 2 tasks (= 2 commits). If a task covers more than one logical change or its `WHERE` scope implies >300 changed lines, split it further.
