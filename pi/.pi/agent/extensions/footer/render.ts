...
const BAR_WIDTH = FOOTER_LAYOUT.contextBarWidth
...
function justifyLine(left: string, right: string, width: number): string {
	const pad = ' '.repeat(
		Math.max(1, width - terminalLineWidth(left) - terminalLineWidth(right)),
	)
...
export function renderFooterLines(input: FooterRenderInput): string[] {
	...
	const modelGroup = [
		...
		fgHex(
			THINKING_COLORS[input.thinkingLevel] ?? LATTE.subtext0,
			`${ICONS.thinking} ${input.thinkingLevel}`,
		),
	].join(` ${fgHex(LATTE.mauve, ICONS.separator)} `)
	...
	if (line1 === undefined) {
		...
		line1 =
			terminalLineWidth(left) + terminalLineWidth(compactQuota) + 2 <=
			width
	...
	for (const right of rights) {
		for (const left of lefts) {
			if (
				terminalLineWidth(left) + terminalLineWidth(right) + 2 <=
				width
			) {
	...
	line2 ??= truncateTerminalLine(
		justifyLine(lefts[0]!, costGroup, width),
		width,
	)
	return [line1, line2]
}
